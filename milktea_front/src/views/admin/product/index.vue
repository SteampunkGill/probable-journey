...
const loadIngredients = async () => {
  const res = await get('/api/admin/inventory/list')
...
const batchStatus = async (status) => {
  await post('/api/admin/products/batch-status', { ids: selectedIds.value, status })
  loadProducts()
}
...
const showRecipeModal = async (p) => {
  ...
  const res = await get(`/api/admin/recipes/${p.id}`)
...
const saveRecipe = async () => {
  await post(`/api/admin/recipes/${recipeModal.value.product.id}`, recipeModal.value.list)
